...
const loginUserController = async(req, res) =>{
    ...
    if(!usuarioEncontrado) return res.status(404).json({msg: "El usuario no se encuentra registrado"})
    
    const confirmPassword =  await usuarioEncontrado.mathPassword(password)
    if(!confirmPassword) return res.status(404).json({msg: "La contraseña es incorrecta vuelvalo a intentar"})
